            ...
            <v-col cols="12">
                <v-card flat class="px-12">
                    <v-form @submit.prevent="onSubmitForm">
                        ...
                        <v-text-field v-model="itemPic.title" dense outlined label="제목" style="width:500px;" :rules="[ v => !!v || '제목은 필수입니다.' ]"></v-text-field>
                        ...
                        <div class="text-center pb-6">
                            ...
                            <v-btn width="80px" class="mx-2" color="blue accent-2" style="color:white" type="submit">수정완료</v-btn>
                        </div>
                    </v-form>
                </v-card>
            </v-col>
        ...
        methods:{
            onSubmitForm(){
                ...
                if (this.type==='notice') {
                    ...
                    oUsernewsDB.child(this.itemPic['.key']).update({
                        'title' : this.itemPic.title,
                        'text' : this.itemPic.text,
                        'writer': this.itemPic.writer,
                        'date' : this.date.split('T')[0]
                    }).then(this.$router.push({name:'usernews_list', params:{page:this.pageNum}}))
                } else if (this.type === 'tips') {
                    oTipsDB.child(this.itemPic['.key']).update({
                        'title' : this.itemPic.title,
                        'text' : this.itemPic.text,
                        'writer': this.itemPic.writer,
                        'date' : this.date.split('T')[0]
                    }).then(this.$router.push({name:'tips_list', params:{page:this.pageNum}}))
                } else if (this.type === 'freeboard') {
                    oFreeboardDB.child(this.itemPic['.key']).update({
                        'title' : this.itemPic.title,
                        'text' : this.itemPic.text,
                        'writer': this.itemPic.writer,
                        'date' : this.date.split('T')[0]
                    }).then(this.$router.push({name:'freeboard_list', params:{page:this.pageNum}}))
                } else if (this.type === 'humor') {
                    oHumorDB.child(this.itemPic['.key']).update({
                        'title' : this.itemPic.title,
                        'text' : this.itemPic.text,
                        'writer': this.itemPic.writer,
                        'date' : this.date.split('T')[0]
                    }).then(this.$router.push({name:'humor_list', params:{page:this.pageNum}}))
                } else if (this.type === 'video') {
                    oVideoDB.child(this.itemPic['.key']).update({
                        'title' : this.itemPic.title,
                        'text' : this.itemPic.text,
                        'writer': this.itemPic.writer,
                        'date' : this.date.split('T')[0]
                    }).then(this.$router.push({name:'video_list', params:{page:this.pageNum}}))
                } else if (this.type === 'finduser') {
                    oFinduserDB.child(this.itemPic['.key']).update({
                        'title' : this.itemPic.title,
                        'text' : this.itemPic.text,
                        'writer': this.itemPic.writer,
                        'date' : this.date.split('T')[0]
                    }).then(this.$router.push({name:'finduser_list', params:{page:this.pageNum}}))
                } else if (this.type === 'report') {
                    oReportDB.child(this.itemPic['.key']).update({
                        'title' : this.itemPic.title,
                        'text' : this.itemPic.text,
                        'writer': this.itemPic.writer,
                        'date' : this.date.split('T')[0]
                    }).then(this.$router.push({name:'report_list', params:{page:this.pageNum}}))
                } else if (this.type === 'picture') {
                    oPictureDB.child(this.itemPic['.key']).update({
                        'title' : this.itemPic.title,
                        'text' : this.itemPic.text,
                        'writer': this.itemPic.writer,
                        'date' : this.date.split('T')[0]
                    }).then(this.$router.push({name:'picture_list', params:{page:this.pageNum}}))
                }
            },
            moveToNoticeList(){
                if (this.type === 'notice') {
