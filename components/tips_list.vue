                ...
                <v-sheet min-height="70vh" rounded="lg" style="background:#FFFFFF" class="mt-3">
                    <v-row>
                        <v-col cols="4" class="ml-8">
                            <h2 class="mt-1" :class="$mq==='pc'? '' : 'text-h6 font-weight-bold'">팁/노하우</h2>
                        </v-col>
                        <v-spacer></v-spacer>
...
<script>
    ...
    import { oTipsDB } from '@/assets/firebase'
    export default {
        firebase : { contentlist : oTipsDB },
        data(){
            return {
                contentlist : [],
                headers:[
        ...
        methods:{
            clickRow(item){
                this.$router.push({name:'board_content', params:{ item:item, title:item.title, page:this.page, type:'tips' }})
            },
            moveToWrite(){
                this.$router.push({name:'board_write', params:{ type:'tips' }})
            }
        },
        created(){
