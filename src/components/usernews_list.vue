                ...
                <v-sheet min-height="70vh" rounded="lg" style="background:#FFFFFF" class="mt-3">
                    <v-row>
                        <v-col cols="4" class="ml-8">
                            <h2 class="mt-1" :class="$mq==='pc'? '' : 'text-h6 font-weight-bold'">유저 뉴스</h2>
                        </v-col>
                        <v-spacer></v-spacer>
...
<script>
    ...
    import { oUsernewsDB } from '@/assets/firebase'
    export default {
        firebase : { contentlist : oUsernewsDB },
        data(){
            return {
                contentlist : [],
                headers:[
        ...
        methods:{
            clickRow(item){
                this.$router.push({name:'board_content', params:{ item:item, title:item.title, page:this.page, type:'usernews' }})
            },
            moveToWrite(){
                this.$router.push({name:'board_write', params:{ type:'usernews' }})
            }
        },
        created(){
